...
<path d="M6.66006 10.875C6.16941 10.875 5.69885 11.0725 5.35191 11.4242C5.00497 11.7758 4.81006 12.2527 4.81006 12.75C4.81006 13.2473 5.00497 13.7242 5.35191 14.0758C5.69885 14.4275 6.16941 14.625 6.66006 14.625H6.67486C7.16551 14.625 7.63606 14.4275 7.98301 14.0758C8.32995 13.7242 8.52486 13.2473 8.52486 12.75C8.52486 12.2527 8.32995 11.7758 7.98301 11.4242C7.63606 11.0725 7.16551 10.875 6.67486 10.875H6.66006Z" fill="var(--md-sys-color-primary)"/>
...
function renderSidebar(adb: Adb): HTMLElement {
  ...
  inner.innerHTML = `
    ...
    <div class="sidebar-device-card" id="sb-device-card">
      <div class="device-card-inner">
        <div class="battery-v-container">
          <div class="battery-v-unfilled" id="sb-batt-v-unfilled"></div>
          <div class="battery-v-fill" id="sb-batt-v-fill" style="height:0%"></div>
        </div>
        <div class="device-card-body">
          <div class="battery-pct-main">
            <span id="sb-batt-pct-num">—</span>
            <div class="battery-right-stack">
              <div class="battery-icon-above">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--md-sys-color-primary)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="transform:rotate(-90deg)">
                  <rect x="2" y="7" width="16" height="11" rx="2"/>
                  <path d="M22 11v3"/>
                  <line x1="6" y1="12.5" x2="12" y2="12.5"/>
                </svg>
              </div>
              <span class="battery-pct-unit">%</span>
            </div>
          </div>
    ...
    <div class="sidebar-disconnect-wrap">
      <button id="btn-disconnect" class="btn-disconnect">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#BA1A1A" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
  ...
  const updateDeviceCard = (device: typeof state.device) => {
    ...
    q("sb-batt-pct-num").textContent = `${device.batteryLevel}`;
    const fill = inner.querySelector<HTMLElement>("#sb-batt-v-fill")!;
    fill.style.height = `${device.batteryLevel}%`;
    const unfilled = inner.querySelector<HTMLElement>("#sb-batt-v-unfilled")!;
    // Calculate unfilled height, keeping a 6px gap when both are visible
    unfilled.style.height = device.batteryLevel >= 100 ? '0' : `calc(${100 - device.batteryLevel}% - 6px)`;
  };
